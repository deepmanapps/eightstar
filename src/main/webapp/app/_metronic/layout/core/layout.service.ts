...
const LAYOUT_CONFIG_LOCAL_STORAGE_KEY = `0.0.0-layoutConfig`;
const BASE_LAYOUT_TYPE_LOCAL_STORAGE_KEY = `0.0.0-baseLayoutType`;
...
export class LayoutService {
  ...
  getLayoutConfig(layoutType: LayoutType): ILayout {
    ...
    return this.getLayoutByType('dark-sidebar');
  }

  setBaseLayoutType(layoutType: LayoutType) {
    const config = this.getLayoutByType('dark-sidebar');
    if (localStorage) {
      localStorage.setItem(BASE_LAYOUT_TYPE_LOCAL_STORAGE_KEY, layoutType);
      localStorage.setItem(`dark-sidebar-${LAYOUT_CONFIG_LOCAL_STORAGE_KEY}`, JSON.stringify(config));
    }
  ...
  resetBaseConfig() {
    ...
    if (localStorage) {
      localStorage.removeItem(`dark-sidebar-${LAYOUT_CONFIG_LOCAL_STORAGE_KEY}`);
    }
